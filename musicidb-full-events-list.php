						...
						<li id="event-<?php esc_attr_e( $event->get_id() ); ?>" class="summaryContent listEvent listItem cf">
							...
							<?php if( !empty( $event->get_more_info() ) ): ?>
								<div class="summaryToggle collapsed">
									<i class="fui-plus-circle" title="Click to Show More"></i>
								</div>
							<?php endif; ?>
							...
							<div class="eventPosterArea">
								...
								<?php if( !empty( $event->get_poster() ) || !empty( $event->get_thumbnail() ) ): ?>
										...
										<img src="<?php echo $thumbnail_src; ?>" class="eventImg imgShadow alignleft" alt="<?php esc_attr_e( $event->get_name() ); ?>" />
											...
											<?php if( !empty( $artist ) ): ?>
												<div class="bandPicBox">
													<?php if( !empty( $artist->get_image_url() ) ): ?>
														<img src="<?php echo musicidb_get_image_at_size( $artist->get_image_url(), 'MediumLarge' ); ?>" class="artistReplace alignleft" />
													<?php endif; ?>
												</div><!-- /.bandPicBox -->
											<?php endif; ?>
										<?php endforeach; ?>
								...
								<?php else: ?>
									<div>
										<img src="<?php echo esc_url( plugins_url('/images/defaultEvent.jpg', MUSICIDB_PLUGIN ), array( 'http', 'https' ) ); ?>" class="artistReplace alignleft defaultPic" />
									</div><!-- /.bandPicBox -->
								<?php endif; ?>
							...
							<div class="listingBody">
								<h3 class="titleofevent">
									...
									<?php $venue = $event->get_venue(); ?>
										...
										<a href="https://www.musicindustrydatabase.com/venue/getVenueDetail.htm?venueId=<?php esc_attr_e( $venue->get_id() ); ?>" title="<?php esc_attr_e( $venue_formatted_address ); ?>" target="_blank" class="venueTitle">
